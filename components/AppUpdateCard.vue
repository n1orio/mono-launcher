<template>
  ...
  class="fixed bottom-4 right-4 z-40 w-80 max-w-[calc(100vw-2rem)] rounded-md  bg-[var(--panel)] p-3.5 shadow-lg shadow-black/40"
  >
  <div class="flex items-start gap-2.5">
  <svg viewBox="0 0 16 16" class="mt-0.5 h-[18px] w-[18px] shrink-0 fill-[var(--accent)]">
  ...
  <div class="text-[13px] font-semibold text-[color:var(--tx-strong)]">
  {{ t("appUpdate.title") }}
  </div>
  <div class="mt-0.5 truncate text-[13px] text-[color:var(--tx-muted)]">
  {{ t("appUpdate.version", { v: appUpdate.version }) }}
  </div>
  <p v-if="appUpdate.notes" class="mt-1 max-h-12 overflow-hidden text-[13px] leading-snug text-[color:var(--tx)]">
  {{ appUpdate.notes.slice(0, 180) }}{{ appUpdate.notes.length > 180 ? "…" : "" }}
  </p>
  <button
  type="button"
  class="mt-2.5 w-full rounded-md bg-[var(--accent-deep)] px-3 py-1.5 text-[13px] font-semibold text-white transition-colors hover:bg-[var(--accent-hover)]"
  ...
  <span class="tabular-nums font-mono text-xs text-[color:var(--tx-muted)]">
  {{ appUpdateProgress ?? 0 }}%
  </span>
  </div>
  ...
  <div class="mt-1.5 text-xs text-[color:var(--tx-muted)]">
  {{ t("appUpdate.restart") }}
  </div>
  </div>
  </div>
