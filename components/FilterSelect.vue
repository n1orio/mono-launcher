<template>
  <div ref="root" class="relative">
    <button
      type="button"
      class="flex max-w-[180px] items-center gap-1.5 rounded-md border border-[var(--border)] bg-[var(--bg)] px-2.5 py-1.5 text-[11px] text-[color:var(--tx)] transition-colors hover:border-[var(--accent)] disabled:opacity-50"
      ...
      <span class="flex min-w-0 items-center gap-1.5">
        <span v-if="selectedLabel" class="flex min-w-0 items-center gap-1.5">
          ...
          <span
            ...
            class="shrink-0 rounded-full bg-[color-mix(in_srgb,var(--accent)_15%,transparent)] px-1.5 text-[10px] font-semibold text-[var(--accent)]"
    ...
    <div
      v-if="open"
      class="absolute left-0 top-[calc(100%+4px)] z-50 flex max-h-64 w-52 flex-col overflow-hidden rounded-md border border-[var(--border)] bg-[var(--panel)] shadow-xl"
    >
      <div class="flex shrink-0 items-center gap-2 border-b border-[var(--border)] px-2 py-1.5">
        ...
        <input
          ...
          class="w-full bg-transparent text-[11px] text-[color:var(--tx)] placeholder-[var(--tx-muted)] outline-none"
        />
        <button
          v-if="modelValue.length"
          type="button"
          class="shrink-0 text-[10px] text-[var(--accent)] hover:underline"
          @click="reset"
        >{{ t("mods.reset") }}</button>
      </div>
      <div class="min-h-0 flex-1 overflow-y-auto p-1">
        <button
          ...
          :key="o.value"
          type="button"
          class="flex w-full items-center gap-2 rounded px-2 py-1 text-left text-[11px] text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)]"
          @click="toggle(o)"
        >
          <span
            class="flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border"
            :class="isOn(o.value) ? 'border-[var(--accent)] bg-[var(--accent)]' : 'border-[var(--border)] bg-[var(--input)]'"
          >
        ...
        <p v-if="!filtered.length" class="px-2 py-1.5 text-[11px] text-[color:var(--tx-muted)]">{{ t("mods.noResults") }}</p>
      </div>
    </div>
  </div>
</template>
