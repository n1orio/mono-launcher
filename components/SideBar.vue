<template>
  ...
  @contextmenu.prevent="openSideCtx($event, p)"
  >
  <span
  v-if="p.id === packId"
  ...
  :title="item.title"
  @click="tab = item.id as typeof tab.value"
  >
  ...
  <!-- Контекстное меню: ПКМ по сборке -->
  <Teleport to="body">
  <div v-if="ctxMenuPack && ctxMenuPos" class="fixed inset-0 z-[70]" @mousedown="closeSideCtx" @contextmenu.prevent="closeSideCtx">
    <div
    ...
    >
      <div class="px-2.5 py-1.5">
        <div class="truncate text-[13px] font-semibold text-[color:var(--tx-strong)]">{{ ctxMenuPack.name }}</div>
      </div>
      <div class="mx-3 border-t border-[var(--border)]"></div>
      <button type="button" class="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-[13px] text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)] disabled:opacity-50" :disabled="busy || gameRunning" @click="openPackTab(ctxMenuPack.id); tab = 'play'; closeSideCtx()">
   <AppIcon name="play" class="h-4 w-4 shrink-0 fill-current" />
         {{ t("side.play") }}
      </button>
      <button type="button" class="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-[13px] text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)]" @click="openPackTab(ctxMenuPack.id); tab = 'settings'; closeSideCtx()">
      ...
      <button type="button" class="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-[13px] text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)]" @click="openPackTab(ctxMenuPack.id); tab = 'news'; closeSideCtx()">
        <svg viewBox="0 0 16 16" class="h-4 w-4 shrink-0 fill-current"><path d="M1.5 3.25A2.25 2.25 0 0 1 3.75 1h8.5A2.25 2.25 0 0 1 14.5 3.25v9.5A2.25 2.25 0 0 1 12.25 15H3.75a2.25 2.25 0 0 1-2.25-2.25v-9.5Zm1.5 0v9.5c0 .414.336.75.75.75h8.5a.75.75 0 0 0 .75-.75v-9.5a.75.75 0 0 0-.75-.75h-8.5a.75.75 0 0 0-.75.75Z"/></svg>
        {{ t("nav.news") }}
      </button>
      <div class="mx-3 border-t border-[var(--border)]"></div>
      <button type="button" class="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-[13px] text-[#f85149] transition-colors hover:bg-[#f85149]/10 disabled:opacity-50" :disabled="busy || gameRunning || removingPack === ctxMenuPack.id" @click="handleRemovePack(ctxMenuPack.id); closeSideCtx()">
...
// ---- Контекстное меню ПКМ по сборке в сайдбаре ----
const ctxMenuPack = ref<{ id: string; name: string } | null>(null);
const ctxMenuPos = ref<{ x: number; y: number } | null>(null);
function openSideCtx(e: MouseEvent, p: { id: string; name: string }) {
  e.preventDefault();
  ctxMenuPack.value = p;
  ctxMenuPos.value = { x: e.clientX, y: e.clientY };
}
function closeSideCtx() {
  ctxMenuPack.value = null;
  ctxMenuPos.value = null;
}
const ctxMenuStyle = computed(() => {
  if (!ctxMenuPos.value) return {};
  const x = Math.min(ctxMenuPos.value.x, window.innerWidth - 220);
  const y = Math.min(ctxMenuPos.value.y, window.innerHeight - 260);
