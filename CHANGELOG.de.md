...
- NeoForge JPMS Absturz behoben: Nur `neoforged/neoforge/` (NeoForge-Mod-JAR) und `net/minecraft/client/` (srg-Client) aus classpath ausgeschlossen — fmlloader/fmlcore/bus/securejarhandler MÜSSEN auf classpath bleiben (sie registrieren forgeclient via ServiceLoader). Kopie des vanilla client jars (versions/neoforge-<ver>/neoforge-<ver>.jar) entfernt, um Duplikatmodul zu vermeiden; nun versions/1.21.1/1.21.1.jar direkt. Auch `${version_name}`/`${version}` zu placeholders hinzugefügt für `--version` aus `version.json`. Vollständiges Java-Launch-Command-Logging hinzugefügt
