...
- Behobene Installation der alten Version: `resolve_latest_url` suchte die Sammlung nur nach URL im Katalog. Beim Hinzufügen über einen direkten .mrpack-Link passte die URL nicht zum Katalog, `resolve_latest_url` gab `None` zurück und die alte (ursprüngliche) Sammlungs-URL wurde verwendet. Fallback über `backend_id` und `mono_pack_detail` hinzugefügt, um den URL der neuesten Version zu erhalten
