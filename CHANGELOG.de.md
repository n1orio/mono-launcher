...
- NeoForge JPMS Absturz behoben: Nur `neoforged/neoforge/` (NeoForge-Mod-JAR) und `net/minecraft/client/` (srg-Client) aus classpath ausgeschlossen — fmlloader/fmlcore/bus/securejarhandler MÜSSEN auf classpath bleiben (sie registrieren forgeclient via ServiceLoader). Vanilla client jar (`1.21.1.jar`) komplett aus NeoForge `-cp` entfernt — ModLauncher findet den patchen client selbst via "production client provider"; das Hinzufügen erzeugt ein Duplikatmodul `_1._21._1`. Auch `${version_name}`/`${version}` zu placeholders hinzugefügt für `--version` aus `version.json`. Vollständiges Java-Launch-Command-Logging hinzugefügt
- Behobene Installation der alten Version: `resolve_latest_url` verließ sich auf `ORDER BY version DESC` in der Datenbank, aber String-Sortierung funktioniert nicht für Semver ("1.10" < "1.2"). Versionen werden jetzt nach `created_at DESC` in Rust sortiert. Fallback über `backend_id` und `mono_pack_detail` hinzugefügt, um Sammlungen aus dem Katalog abzurufen, die über einen direkten Link hinzugefügt wurden
- Behobene Anzeige der installierten Versionen (alte zuerst): `mrpack::installed_details()` sortierte `InstalledVersion` aufsteigend nach `version_id`. Auf Absteigend geändert, damit die neueste installierte Version zuerst angezeigt wird
- Pack-Update-Hang beim Entpacken behoben: `extract_mrpack` hatte `let mut total: u64 = 0`, das `let total = archive.len()` überschrieb. `file_total`/`total`/`current` in `DownloadProgress` zeigten Byte des aktuellen Files statt Gesamtzahl/Byte, wodurch die UI 100% zeigte während Entpacken noch lief. `file_total` zählt jetzt nur Dateien (ohne Verzeichnisse), der finale 100%-Fortschritt wird immer nach der Schleife gesendet. Auch `handleInstall` und `installRemoteVersion` setzen jetzt `progress.value = null` in `finally`, damit das Lade-Widget geschlossen wird und die Taste zu «Spielen» zurückkehrt
