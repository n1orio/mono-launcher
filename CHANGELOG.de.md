...
- NeoForge JPMS Absturz behoben: Nur `neoforged/neoforge/` (NeoForge-Mod-JAR) und `net/minecraft/client/` (srg-Client) aus classpath ausgeschlossen — fmlloader/fmlcore/bus/securejarhandler MÜSSEN auf classpath bleiben (sie registrieren forgeclient via ServiceLoader). Client-JAR umbenannt von `neoforge-<ver>.jar` zu `client.jar`, damit JPMS PathBasedLocator keinen automatischen Modul `neoforge` aus dem Dateinamen erstellt. Auch `${version_name}`/`${version}` zu placeholders hinzugefügt für `--version` aus `version.json`. Vollständiges Java-Launch-Command-Logging hinzugefügt
