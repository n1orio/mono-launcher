...
- **Контент из репозитория сборки**: вкладки «Скриншоты» (локальная папка игрока) и «Сервера» (`servers.json` + свои из `servers.dat`), звёзды и соцсети (`socials.json`) в шапке — кнопками с цветом автора (`color` в `socials.json`), статусы серверов, «Играть на сервере», тема лаунчера под стиль сборки (`theme.json` — плавно перекрашивает интерфейс при открытии).
