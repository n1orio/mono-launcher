...
- **Контент из репозитория сборки**: вкладки «Скриншоты» (локальная папка игрока) и «Сервера» (`servers.json` + свои из `servers.dat`), звёзды репозитория в шапке, статусы серверов и «Играть на сервере».
...
| `pack_repo_content(packId?)` | Звёзды и сервера (`servers.json`) сборки |
