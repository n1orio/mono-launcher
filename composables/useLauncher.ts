...
export function useLauncher(options: { keepPackId?: boolean } = {}) {
  ...
  const { themeLevel, applyThemeLevel, setThemeLevel, packThemeActive, setPackThemeVars, toggleTheme } = useTheme();
  ...
  async function refreshSkin() {
    ...
    if (s.user_type !== "microsoft" || !s.uuid) {
      skinUrl.value = "";
      return;
    }
    skinUrl.value = `https://mc-heads.net/avatar/${s.uuid}/128`;
  }
