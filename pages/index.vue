<template>
  ...
  <button
  type="button"
  class="rounded-md  bg-[var(--input)] px-2.5 py-1.5 text-[13px] font-medium text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)] hover:text-white"
  ...
  <button
  type="button"
  class="flex items-center gap-1 rounded-md  bg-[var(--input)] px-2.5 py-1.5 text-[13px] font-medium text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)] hover:text-white"
  ...
  <button
  type="button"
  class="rounded-md  bg-[var(--input)] px-2 py-1 text-[13px] font-medium text-[color:var(--tx-muted)] transition-colors hover:bg-[var(--hover)] hover:text-white"
  ...
  <button
  type="button"
  class="flex items-center gap-1 rounded-md  bg-[#f85149]/10 px-2 py-1 text-[13px] font-semibold text-[#f85149] transition-colors hover:bg-[#f85149]/20 disabled:opacity-50"
  ...
  <button
  type="button"
  class="flex items-center gap-1 rounded px-2 py-1 text-[13px] font-semibold transition-colors"
  ...
  type="button"
  class="rounded px-1.5 py-1 text-[13px] leading-none text-[color:var(--tx-muted)] transition-colors hover:text-[var(--accent)]"
  ...
  type="button"
  class="flex items-center gap-1 rounded px-2 py-1 text-xs font-semibold text-white bg-[var(--accent)] transition-colors hover:opacity-90"
  ...
  class="w-full rounded-md  bg-[var(--bg)] py-1.5 pl-7 pr-2 text-[13px] text-[color:var(--tx)] placeholder-[var(--tx-muted)] outline-none transition-colors "
  />
  </div>
  <div ref="fileMenuRef" class="relative">
  <button
  type="button"
  class="flex shrink-0 items-center rounded-md  bg-[var(--input)] px-2.5 py-1 text-[13px] font-medium text-[color:var(--tx)] transition-colors hover:bg-[var(--hover)] hover:text-white"
  :title="t('files.more')"
...
<span class="flex items-center gap-1.5 text-[13px] font-medium text-[color:var(--tx-strong)]">
  ...
  :class="c.myRating === 1 ? '/50 bg-[#3fb950]/15 text-[#3fb950]' : ' bg-[var(--input)] text-[color:var(--tx-muted)] hover:text-[#3fb950]'"
  :title="t('comments.like')"
  @click="rateCatalogComment(catalogDetail.id, c.id, 1)">
  👍 {{ c.likes }}
  </button>
  <button type="button" class="flex items-center gap-1 rounded  px-1.5 py-0.5 text-xs font-semibold transition-colors"
  :class="c.myRating === -1 ? '/50 bg-[#f85149]/15 text-[#f85149]' : ' bg-[var(--input)] text-[color:var(--tx-muted)] hover:text-[#f85149]'"
  :title="t('comments.dislike')"
  @click="rateCatalogComment(catalogDetail.id, c.id, -1)">
  👎 {{ c.dislikes }}
  </button>
  ...
  <div class="ml-auto flex shrink-0 items-center gap-1">
  <button type="button" class="flex items-center gap-1 rounded  px-1.5 py-0.5 text-xs font-semibold transition-colors"
  :class="r.myRating === 1 ? '/50 bg-[#3fb950]/15 text-[#3fb950]' : ' bg-[var(--input)] text-[color:var(--tx-muted)] hover:text-[#3fb950]'"
  @click="rateCatalogComment(catalogDetail.id, r.id, 1)">
  👍 {{ r.likes }}
  </button>
  <button type="button" class="flex items-center gap-1 rounded  px-1.5 py-0.5 text-xs font-semibold transition-colors"
  :class="r.myRating === -1 ? '/50 bg-[#f85149]/15 text-[#f85149]' : ' bg-[var(--input)] text-[color:var(--tx-muted)] hover:text-[#f85149]'"
  @click="rateCatalogComment(catalogDetail.id, r.id, -1)">
  👎 {{ r.dislikes }}
  </button>
  </div>
  ...
  :key="v.id"
  class="flex items-center gap-3 rounded-md  bg-[var(--bg)] px-3 py-2"
  >
  <div class="min-w-0 flex-1">
  ...
  :class="scanResult.safe ? '/40 bg-[#3fb950]/5' : '/50 bg-[#f85149]/10'">
  <div class="flex items-center gap-2">
...
<FilterSelect
  ...
  <!-- Оверлей drag&drop: отпустите .mrpack, чтобы добавить сборку -->
  <div
  v-if="dragMrpack"
  class="pointer-events-none fixed inset-0 z-[90] flex items-center justify-center bg-[color-mix(in_srgb,var(--bg)_75%,transparent)]"
  >
  <div class="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-[var(--border)] bg-[var(--panel)] px-10 py-8 shadow-2xl">
  <svg viewBox="0 0 16 16" class="h-8 w-8 fill-current text-[var(--accent)]"><path d="M1 7.775V2.75C1 1.784 1.784 1 2.75 1h5.025c.464 0 .91.184 1.238.513l6.25 6.25a1.75 1.75 0 0 1 0 2.474l-5.026 5.026a1.75 1.75 0 0 1-2.474 0l-6.25-6.25A1.752 1.752 0 0 1 1 7.775Z"/></svg>
  <span class="text-sm font-semibold text-[color:var(--tx-strong)]">{{ t("library.dropMrpack") }}</span>
  </div>
  </div>
</template>
...
async function handleDroppedPaths(paths: string[]) {
  const mrpacks = paths.filter((p) => p.toLowerCase().endsWith(".mrpack"));
  if (mrpacks.length === 0) return;
  for (const p of mrpacks) {
  try {
  const added = await addPackFile(p);
  notify(t("library.addedFile", { name: added.name }), "success");
  } catch (e) {
  notify(String(e));
  }
  }
...
/** Подписка на drag&drop файлов (Tauri): показываем оверлей, ловим .mrpack. */
async function initDragDrop() {
  ...
  dragMrpack.value = true;
  } else if (event.payload.type === "drop") {
  dragDepth = 0;
  dragMrpack.value = false;
  ...
  // webview API недоступен — drag&drop просто не работает.
  }
}
...
async function pickAuthorVersionFile() {
  ...
  authorVersionFile.value = picked;
  const base = picked.split(/[\\/]/).pop()?.replace(/\.(mrpack|zip)$/i, "") ?? "";
  const m = base.match(/(\d+(?:\.\d+){1,3}(?:[-+][\w.]*)?)$/);
  if (m && !authorNewVersion.value.trim()) authorNewVersion.value = m[1];
  }
}

/** Публикует новую версию; при успехе очищает форму. */
async function submitAuthorVersion() {
  const ok = await createAuthorVersion(authorVersionFile.value, authorNewVersion.value.trim(), authorNewChangelog.value);
  if (ok) {
  authorVersionFile.value = "";
  authorNewVersion.value = "";
  authorNewChangelog.value = "";
  }
}
...
const authorShotCaption = ref("");

interface AuthorShot { url: string; caption: string }
const authorShots = computed<AuthorShot[]>(() => {
  ...
  .map((s) =>
    typeof s === "string"
      ? { url: s, caption: "" }
      : { url: (s as { url?: string } | null)?.url ?? "", caption: (s as { caption?: string } | null)?.caption ?? "" }
  )
  .filter((s) => s.url);
});

function authorShotsPayload(next: unknown[]): Record<string, unknown> {
...
async function addAuthorShot() {
  ...
  const caption = authorShotCaption.value.trim();
  authorShotUrl.value = "";
  authorShotCaption.value = "";
  await updateAuthorMeta({ meta: authorShotsPayload([...rawShots(), caption ? { url, caption } : url]) });
}

/** Сырой массив meta.screenshots как есть. */
function rawShots(): unknown[] {
  const meta = (authorDetail.value?.meta as Record<string, unknown> | null) ?? {};
  return Array.isArray(meta.screenshots) ? [...(meta.screenshots as unknown[])] : [];
}

/** Меняет подпись скриншота (сохраняет объектную форму записи). */
async function setShotCaption(index: number, caption: string) {
  ...
  if (typeof cur === "string") {
    if (!cap) return;
    shots[index] = { url: cur, caption: cap };
  } else {
    const obj = { ...(cur as Record<string, unknown>) };
    if (cap) obj.caption = cap;
    else delete obj.caption;
    shots[index] = obj;
  }
  await updateAuthorMeta({ meta: authorShotsPayload(shots) });
}

/** Удаляет скриншот по индексу через бэкенд (чистит файл на storage). */
async function removeAuthorShot(index: number) {
  if (!authorDetail.value) return;
  await deleteAuthorScreenshot(index);
}

/** Меняет порядок скриншотов местами (сохраняя исходную форму записей). */
async function moveAuthorShot(index: number, dir: -1 | 1) {
  if (!authorDetail.value) return;
  const meta = (authorDetail.value.meta as Record<string, unknown> | null) ?? {};
  const shots = Array.isArray(meta.screenshots) ? [...(meta.screenshots as unknown[])] : null;
  if (!shots) return;
  const j = index + dir;
  if (j < 0 || j >= shots.length) return;
  [shots[index], shots[j]] = [shots[j], shots[index]];
  await updateAuthorMeta({ meta: { ...meta, screenshots: shots } });
}

/** Диалог выбора картинки → загрузка на storage (с подписью из поля ниже). */
async function pickAuthorShotFile() {
  const picked = await openDialog({
  filters: [{ name: "Image", extensions: ["png", "jpg", "jpeg", "webp", "gif"] }],
  });
  if (typeof picked === "string") {
  const ok = await uploadAuthorScreenshot(picked, authorShotCaption.value.trim());
  if (ok) authorShotCaption.value = "";
  }
}

// ---- Форма «Обзор»: баннер, dirty-tracking, опасная зона ----
const authorOverviewBanner = ref("");
watch(authorDetail, (d) => {
  const m = d?.meta as Record<string, unknown> | null | undefined;
  authorOverviewBanner.value = m && typeof m.banner === "string" ? m.banner : "";
}, { immediate: true });
...
const authorDelArm = ref(false);
let authorDelTimer: ReturnType<typeof setTimeout> | null = null;
function armAuthorDelete() {
  if (authorDelArm.value) {
  authorDelArm.value = false;
  if (authorDelTimer) clearTimeout(authorDelTimer);
  void deleteAuthorPack();
  return;
  }
  authorDelArm.value = true;
  if (authorDelTimer) clearTimeout(authorDelTimer);
  authorDelTimer = setTimeout(() => (authorDelArm.value = false), 5000);
}
...
/** Иконки вкладок настроек (viewBox 16). */
const SETTINGS_TAB_ICONS: Record<"accounts" | "appearance", string> = {
  accounts: '<path d="M8 1a3 3 0 1 0 0 6 3 3 0 0 0 0-6ZM2 13.25C2 10.75 4.46 9.25 8 9.25s6 1.5 6 4V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-.75Z"/>',
  appearance: '<path fill-rule="evenodd" d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1Zm0 1.5v11a5.5 5.5 0 0 1 0-11Z"/>',
};
