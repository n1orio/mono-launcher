        ...
        <div class="relative flex h-full w-full">
          <div class="relative h-full min-w-0 flex-1 overflow-hidden">
            <div class="mx-auto flex h-full w-full flex-col px-4 py-6 sm:px-6 md:px-8">
              <TabPlay v-if="tab === 'play'" />
              <TabNews v-else-if="tab === 'news'" />
              <TabCatalog v-else-if="tab === 'catalog'" />
              <TabDev v-else-if="tab === 'dev'" />
              <TabLibrary v-else-if="tab === 'library'" />
              <TabAuthor v-else-if="tab === 'author'" />
              <TabAdmin v-else-if="tab === 'admin'" />
              <TabSettings v-else />
            </div>
          </div>
        </div>
...
const loginMethod = ref<"mono" | "ely">("mono");
const loginBusy = monoBusy;
function doLogin() { handleMonoLogin(); }

const boostyToken = ref("");
const boostySecret = ref("");
const boostyKey = ref("");
const boostyBusy = ref(false);
const boostyError = ref("");
async function doBoostyAuth() {
  boostyBusy.value = true;
  boostyError.value = "";
  try {
    const token = boostyToken.value.trim();
    const secret = boostySecret.value.trim();
    const key = boostyKey.value.trim();
    if (!token || !secret || !key) return;
    boostyGlobalOpen.value = true;
  } catch (e: any) {
    boostyError.value = String(e);
  } finally {
    boostyBusy.value = false;
  }
}
...
function applyAutoFilters() {
  ...
  if (loader && CF_LOADER_MAP[loader]) {
  curseLoaderType.value = CF_LOADER_MAP[loader];
  } else {
...
/** Маппинг имени загрузчика на CurseForge modLoaderType ID. */
