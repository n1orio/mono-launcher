...
- **Signing required**: `bundle.createUpdaterArtifacts: true` → build fails without minisign key. Local: `export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/nio-launcher.key)"` + `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`
...
- `assets/css/main.css` — Tailwind entry + **theme CSS variables** (`--bg`, `--panel`, `--input`, `--border`, `--tx*`, `--accent*`, alpha variants). **Never hardcode colors**; theme toggled via `.theme-light` on `documentElement`, persisted in `localStorage.nio.theme`
...
## Key Features
- **Playtime/Achievements**: `.nio-playtime.json` per version. `AppStatus` has `playtime_seconds`, `total_playtime_seconds`, `played_packs`
...
- **CurseForge**: API v1 needs `x-api-key` (file `<root>/curseforge-key.txt` / `NIO_CURSEFORGE_KEY`). Search opens in separate Tauri WebviewWindow (`search`, URL `?win=search&...`). Results sent to main via `mods-changed` event
...
- **Deep links**: Scheme `niol` (`niol://add-pack?url=&name=&blog=`). Handled via `deep-link://new-url` listener + argv scan at startup. Duplicates filtered via `HANDLED_LINKS`. UI notified via `pack-added` event
- **Universal invite**: `https://n1orio.github.io/nio-launcher/?url=&name=` (gh-pages branch). Auto-opens `niol://`, falls back to download CTA
- **Custom mods / integrity**: Trusted hosts = Modrinth CDNs + `mediafiles.forgecdn.net`/`edge.forgecdn.net`. All files hash-verified (sha1/sha512). Untrusted + `overrides` jars → `.nio-custom.json` per version → `AppStatus.custom_mods` + warning banner (disable via `warn-custom-mods.txt`)
...
- **News**: Pack releases + discussions + launcher's own releases (`NEWS_REPO = n1orio/nio-launcher`, `pack_id: "launcher"`). Launcher news items show "open release page" button
- **UI**: Each pack has its own nav tab (`openPackTab`); no sidebar `<select>`
- **Per-pack data**: `~/.local/share/NioLauncher/packs/<pack_id>/` (versions/, active.json, mrpack-cache/); shared runtime in root
...
- Requires `TAURI_SIGNING_PRIVATE_KEY` + `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` + `NIO_CURSEFORGE_KEY` secrets
...
- E2E deep-link (Linux): after `tauri build`, verify `xdg-mime query default x-scheme-handler/niol`, then `xdg-open "niol://add-pack?..."` under Xvfb, inspect `~/.local/share/NioLauncher/packs.json`
